...
```
1. 학생이 강좌를 검색하여 주문한다.
2. 학습매니저가 선정되고 학습관리를 시작한다. 
3. 학습매니저가 학습평가를 진행한다.
4. 학습종료시 학습매니저가 최종학습평가를 등록한다.
5. 최종학습평가점수는 주문관리에도 반드시 등록해야만 한다.
6. 학생이 강좌주문을 취소하면 학습관리와 학습평가도 취소된다.
7. 강좌주문, 강좌주문취소, 학습관리, 학습관리취소는 학생에게 SMS를 발송한다.
8. 학생은 학습현화을 조회 할 수 있다.
```

### 비기능에 낙찰자 정보가 등록되어야 한다. (Sync 호출)
1. 트랜잭션
  - 평가결과가 등록되면 주문관리에 평가결과점수가 등록되어야 한다. (Sync 호출)
2. 장애격리
  - 학습관리 기능이 수행되지 않더라도 주문관리, 학습평가 기능은 365일 24시간 받을 수 있어야 한다. Async (event-driven), Eventual Consistency
  - 주문관리 기능이 과중되면 사용자를 잠시 동안 받지 않고 주문을 잠시후에 하도록 유도한다. Circuit breaker, fallback
3. 성능
